...
const Checkout = () => {
  ...
  const { order, razorpayOrder, loading } = useSelector(
    (state) => state.orders
  );
  ...
  const handleRazorpayPayment = useCallback(() => {
    if (!razorpayOrder || !order || !window.Razorpay) {
      return;
    }
  ...
  /*
   * Start Razorpay when order is ready
   */
  useEffect(() => {
    if (
      razorpayOrder &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        // Razorpay payment will be handled
        // automatically by the useEffect above.
      } else {
        dispatch(clearCart());
